...
def busqueda_en_haz_backtracking(B, initial_state, memory, goal_state):
    ...
    while len(BEAM) != 0:  # loop until the BEAM contains no nodes
        ...
        if not leafBacktracking:
            ...
            if len(SETtemp) == 0 or fullMemory:
                ...
                # Comprobamos que hay m�s hojas para recorrer en su nivel (hermanos)
                ...
                if hayMasElementos:
                    ...
                    if fullMemory:
                        print("------------------------------- BACKTRACKING HACIA HERMANO (memory) ----------------------------------")
                    else:
                    ...
                    if fullMemory:
                        print("------------------------------- BACKTRACKING HACIA ABUELO (memory) ----------------------------------")
                    else:
                ...
                if fullMemory:
                    print("hash_table: %s" % (hash_table[-5:]))
                    nToRemove = statesToRemove
                    if statesToRemove == 0:
                        nToRemove = B
                    count5 = 0
                    print("nToRemove: %s" % (nToRemove))
                    while count5 < nToRemove:
                        hash_table.pop()
                        print("borrado")
                        count5 = count5 + 1
                    if not leafBacktracking:
    ...
    return "Hemos llegado al final del �rbol. Coste: %s" % (g)
